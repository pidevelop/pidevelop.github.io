...
//use old chart instead of new chart to prevent memory leak
function oldChart(hqChart,varItem){
   for(var old=-1,i=0;i<hqChart.OldPaint.length;i++)if(hqChart.OldPaint[i].Name==varItem.Name) old=i;
   if(old<0) return false;
   else return hqChart.OldPaint[old];
}
...
//        Complier:{ Log:console.log }    //编译器日志
        Chart:{ Log:mylog },      //图形日志
        Complier:{ Log:mylog }    //编译器日志
    };
}
...
function JSAlgorithm(errorHandler,symbolData)
{
    ...
    */
    ...
    this.SLOPE=function(data,n)
    {
        ...
        let x,y,xy,xx;
        for(var i=start+n-1;i<data.length;++i)
        {
            result[i]=null;
            x=y=xy=xx=0;
    ...
    */
    ...
    this.COST=function(data)
    {
        ...
        for(var i=this.SymbolData.Data.Data.length-1, j=0,k=0 ; i>=0; --i)
        {
            result[i]=null;
...
JSDraw.prototype.CalculateDrawLine=function(lineCache)
{
    ...
    {
        for(var i=1;i<lineCache.List.length-1;++i)
        {
...
//        this.VarTable=new Map();
        ...
        for(var i in this.Arguments)    //预定义的变量
        {
        ...
        for(var i in this.AST.Body)
        {
            ...
            if (item.Type==Syntax.ExpressionStatement && item.Expression)
            {
                ...
                else if (item.Expression.Type==Syntax.SequenceExpression)
                {
                    ...
                    for(var j in item.Expression.Expression)
                    {
...
return this.Algorithm.CallFunction(funcName, args, node, this.SymbolData);
...
execute.VarTable.clear();
execute.ConstVarTable.clear();

for(var i=0;i<execute.OutVarTable.length;i++) execute.OutVarTable[i].Data=[];

delete execute.SymbolData.Execute;
delete execute.SymbolData.Data;
delete execute.SymbolData.SourceData;
delete execute.SymbolData;
delete execute.OutVarTable;
...
function ScriptIndex(name,script,args,option)
{
    ...
    {
//        let line=new ChartLine();
let line = oldChart(hqChart,varItem) || new ChartLine();
...
let line = oldChart(hqChart,varItem) || new ChartSubLine();
...
let bar = oldChart(hqChart,varItem) || new ChartStickLine();
...
let chartText = oldChart(hqChart,varItem) || new ChartSingleText();
...
let chartMACD = oldChart(hqChart,varItem) || new ChartMACD();
...
let pointDot = oldChart(hqChart,varItem) || new ChartPointDot();
...
let chart = oldChart(hqChart,varItem) || new ChartStick();
...
let chart = oldChart(hqChart,varItem) || new ChartLineStick();
...
let chart = oldChart(hqChart,varItem) || new ChartVolStick();
...
let chart = oldChart(hqChart,varItem) || new ChartBand();
...
let chart = oldChart(hqChart,varItem) || new ChartLineArea();
...
let chart = oldChart(hqChart,varItem) || new ChartFillRGN();
...
let chart = oldChart(hqChart,varItem) || new ChartFillBGRGN();
...
let chart = oldChart(hqChart,varItem) || new ChartFLOATRGN();
...
let chart = oldChart(hqChart,varItem) || new ChartKLine();
...
let line = oldChart(hqChart,varItem) || new ChartSlopeLine();
...
let chart = oldChart(hqChart,varItem) || new ChartBackground();
...
let chart = oldChart(hqChart,varItem) || new ChartTextLine();
...
let chart = oldChart(hqChart,varItem) || new ChartChannel();
...
let chart = oldChart(hqChart,varItem) || new ChartPartLine();
...
let chart = oldChart(hqChart,varItem) || new ChartMultiLine();
...
let chart = oldChart(hqChart,varItem) || new ChartMultiBar();
...
let chart = oldChart(hqChart,varItem) || new ChartMultiText();
...
let chart = oldChart(hqChart,varItem) || new ChartMultiSVGIcon();
...
let chart = oldChart(hqChart,varItem) || new ChartColorKLine();
...
let chart = oldChart(hqChart,varItem) || new ChartRectangle();
...
let chart = oldChart(hqChart,varItem) || new ChartOverlayLine();
    ...
    {
//        let chart=new ChartKLine();
let chart = oldChart(hqChart,"Self Kline") || new ChartKLine();
...
function OverlayScriptIndex(name,script,args,option)
{
    ...
    this.CreateLine=function(hqChart,windowIndex,varItem,id)
    {
debug&&console.log("CreateOverlayLine");
    ...
    this.CreateStraightLine=function(hqChart,windowIndex,varItem,id)
    {
debug&&console.log("CreateOverlayStraightLine");
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        let chart=new ChartLine();
        chart.DrawType=1;
    ...
    this.CreatePolyLine=function(hqChart,windowIndex,varItem,id)
    {
debug&&console.log("CreateOverlayPolyLine");
        var overlayIndex=this.OverlayIndex;
        var frame=overlayIndex.Frame;
        let chart=new ChartLine();
        chart.Canvas=hqChart.Canvas;
