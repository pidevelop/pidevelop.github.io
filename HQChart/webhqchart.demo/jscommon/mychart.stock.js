var	ChartApi = "ChartApi.php",
	ChartApiAll	= ChartApi + '?All',
	ChartApiBuySell	= ChartApi + '?BuySell',
	ChartApiDeal	= ChartApi + '?Deal',
	ChartApiBase	= ChartApi + '?Base',
	ChartApiFinance	= ChartApi + '?Finance',
	ChartApiDerivative = ChartApi + '?Derivative';

function suffix(o) {
   o.symbol = "300782.sz", o.name = ""; return o;
}

function getalldata() {
if(window.stopdata)return;
if(typeof getlastminute=='undefined')return;
   var ts=new Date().getTime(), ls=localStorage, v=ls.value;
window.stopdata=true;  //prevent reentrant

//   var l=v?v.substr(parseInt(v.substr(0,4),16)+4):"0";
//   var c=Number(l.substr(0,1));
         ...
         $.ajax({
             ...
             {
		"time":getlastminute(),
		"data":v?v.substr(parseInt(v.substr(0,4),16)+4):""
//		"userid":l.substr(1,c),
//		"ent":l.substr(c+1)
             },
             type:"post",
...
//console.log("getalldata",data);
		data.stock[0].minute = updatelastminute(data.stock[0].minute);
   var that=window.JSStock, arySymbol=[];
   for(var item of that.MapStock)arySymbol.push(item[0]);
   that.RequestBuySellData(arySymbol, data);
   that.RequestDealData(arySymbol, data);
   window.RequestMinuteData_obj&&RequestMinuteData_obj.RequestMinuteData(data);
   window.ReqeustHistoryMinuteData_obj&&ReqeustHistoryMinuteData_obj.RecvMinuteHistoryData(suffix({"data":minutedata}));

   ls.values=data.values;
   ls.data=JSON.stringify({
	"price":_latest(_close),
	"sqltime":data.sqltime,
	"nettime":(te-ts)/1000 });

window.stopdata=false;
             },
...
getalldata();
    ...
    this.RequestBaseData=function(arySymbol)
    {
        ...
        if (this.NetworkFilter)
        {
            var obj=
            {
                ...
                Request:{ Url:ChartApiBase, Data:{ field:field, symbol:arySymbol }, Type:'POST' }, 
                Self:this,
    ...
    this.RequestDerivativeData=function(arySymbol)
    {
        ...
        if (this.NetworkFilter)
        {
            var obj=
            {
                ...
                Request:{ Url:ChartApiDerivative, Data:{ field:field, symbol:arySymbol }, Type:'POST' }, 
                Self:this,
                PreventDefault:false
            };
            this.NetworkFilter(obj, function(data) 
    ...
    this.RequestFinanceData=function(arySymbol)
    {
        ...
        if (this.NetworkFilter)
        {
            var obj=
            {
                ...
                Request:{ Url:ChartApiFinance, Data:{ field:field, symbol:arySymbol }, Type:'POST' }, 
                Self:this,
                PreventDefault:false
            };
            this.NetworkFilter(obj, function(data) 
    ...
    this.RequestCapitalData=function(arySymbol)
    {
     ...
     //请求买卖盘
     this.RequestBuySellData=function(arySymbol, data)
     {
...
console.log('------------------------------------JSStock::RequestBuySellData---------买卖盘数据');
...
/*
        if (this.NetworkFilter)
        {
            var obj=
            {
                ...
                Request:{ Url:ChartApiBuySell, Data:{ field:field, symbol:arySymbol }, Type:'POST' }, 
                Self:this,
                PreventDefault:false
            };
            this.NetworkFilter(obj, function(data) 
...
self.RecvData(data,RECV_DATA_TYPE.BUY_SELL_DATA);
     ...
     //请求分笔
     this.RequestDealData=function(arySymbol, data)
     {
...
console.log('------------------------------------JSStock::RequestDealData----------分笔数据');
...
/*
        if (this.NetworkFilter)
        {
            var obj=
            {
                ...
                Request:{ Url:ChartApiDeal, Data:{ field:field, symbol:arySymbol }, Type:'POST' }, 
                Self:this,
                PreventDefault:false
            };
            this.NetworkFilter(obj, function(data) 
...
var that=this,millisec=0;
if(!this.Interval) this.Interval=setInterval(function(){
  if(!window.chartdata)return;
  millisec+=100;
  if(millisec>=2000) millisec=0,getalldata();
  if(localStorage.buysell=="on") $("#rightMinute2").show(); else $("#rightMinute2").hide();
  var cc=localStorage.chartcontrol, chart=(chartdata.viewperiod=='00'?chartdata.Mchart:chartdata.Kchart);
  localStorage.chartcontrol="";
  switch(cc){
    case "zoomup":
	zoomup(chart);
	break;
