...
window.Jtime=0; //prevent double calling loadSectionTpl
...
if(t-Jtime<1000) return;
Jtime=t;
console.log('loadSectionTpl');

function Jcall(section){
 switch(section){
  case "friend_section": get_friend();break;
  case "refresh_section": get_order();break;
  case "leader_section": get_leader();break;
  case "transaction_section": get_transaction();break;
  case "block_section": get_block();break;
  case "account_section": get_account();break;
  default:
 }
}
...
console.log("old page",$(hash)[0].id);
Jcall($(hash)[0].id);
...
$('body').append('<div id="jingle_popup"></div><div id="jingle_popup_mask" class="popup_mask1"></div><div id="jingle_popup_mask" class="popup_mask2"></div><div id="jingle_popup_mask" class="popup_mask3"></div><div id="jingle_popup_mask" class="popup_mask4"></div>');
_mask = $('[id=jingle_popup_mask]');
        _popup = $('#jingle_popup');
        _subscribeEvents();
    }
    ...
    function Refresh(selector,type,callback){
        var iscroll, scroller,refreshEl,iconEl,labelEl,topOffset,isPullDown,
            options = {
                ...
                pullText: "下拉刷新...",
                releaseText: "松开立即刷新...",
                refreshText: lang?translation['刷新中...'][transindex[lang]]:'刷新中...',
...
refreshCache[jRefreshId].scroller.scrollTo(0,0);
